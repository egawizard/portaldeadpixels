...
async function fetchJson(url,timeout=9000,headers={}){
  ...
  for(let attempt=0;attempt<2;attempt++){
    ...
    try{
      const r=await fetch(url,{headers:{accept:"application/json","user-agent":"Mozilla/5.0 (compatible; DEAD-PIXELS-GLITCH-ALPHA/3.7.6; +https://www.deadpixelslabs.com/)",...headers},signal:ctrl.signal});
...
function scorePool(p){
  ...
  score=clamp(score);
  return {score,components:{momentum:Math.round(momentum),liquidity:Math.round(liquidity),activity:Math.round(activity),flow:Math.round(flow),novelty:Math.round(maturity),structure:Math.round(structure)},verdict:score>=82?"ALPHA":score>=70?"HOT":score>=55?"ACTIVE":score>=40?"WATCH":"COLD",model:"V3.7.6"};
}
...
async function detail(address,pairAddress=null,sourceHint=null){
  ...
  const gl=market?scorePool(market):{score:0,verdict:"UNPRICED",components:{momentum:0,liquidity:0,activity:0,flow:0,novelty:0,structure:0},model:"V3.7.6"};gl.scoreScope="MARKET_PAIR";
