...
async function fetchJson(url,timeout=9000,headers={}){
  ...
  for(let attempt=0;attempt<2;attempt++){
    ...
    try{
      const r=await fetch(url,{headers:{accept:"application/json","user-agent":"dead-pixels-glitch-alpha/3.7.3",...headers},signal:ctrl.signal});
...
function scorePool(p){
  ...
  score=clamp(score);
  return {score,components:{momentum:Math.round(momentum),liquidity:Math.round(liquidity),activity:Math.round(activity),flow:Math.round(flow),novelty:Math.round(maturity),structure:Math.round(structure)},verdict:score>=82?"ALPHA":score>=70?"HOT":score>=55?"ACTIVE":score>=40?"WATCH":"COLD",model:"V3.7.3"};
}
...
async function getBlockscoutDetail(address){
  ...
  const out={address:s[0].status==="fulfilled"?s[0].value:null,token:s[1].status==="fulfilled"?s[1].value:null,counters:s[2].status==="fulfilled"?s[2].value:null,holders:null,source:null,search:null,holderSources:{}};
  const extra=await Promise.allSettled([
    cachedJson(`${BLOCKSCOUT}/api/v2/tokens/${a}/holders`,20000,10000),
    legacyTokenHolders(a,500),
    cachedJson(`${BLOCKSCOUT}/api/v2/smart-contracts/${a}`,60000,9000)
  ]);
  const v2=extra[0].status==="fulfilled"?normalizeHolderBody(extra[0].value):null;
  const legacy=extra[1].status==="fulfilled"?extra[1].value:null;
  out.holderSources={v2:v2?.items?.length||0,legacy:legacy?.items?.length||0};
  out.holders=(v2?.items?.length?v2:null)||(legacy?.items?.length?legacy:null)||v2||legacy||null;
  out.source=extra[2].status==="fulfilled"?extra[2].value:null;
...
async function dexPaidIntel(address,market){
  const a=lower(address);if(!addr(a))return {status:"UNKNOWN",paid:null,orders:[],boosts:n(market?.boosts)};
  try{
    const body=await cachedJson(`${DEX}/orders/v1/${NETWORK}/${a}`,60000,9000);
    const orders=Array.isArray(body)?body:[];const paidOrders=orders.filter(x=>n(x.paymentTimestamp)>0&&!['cancelled','rejected'].includes(String(x.status||'').toLowerCase()));
    return {status:paidOrders.length?"PAID":"NOT PAID",paid:!!paidOrders.length,orders:orders.slice(0,12),boosts:n(market?.boosts),source:"DEXSCREENER ORDERS API"};
  }catch(e){return {status:"UNKNOWN",paid:null,orders:[],boosts:n(market?.boosts),source:"DEXSCREENER ORDERS API",error:e?.message||"CHECK_FAILED"};}
}
...
async function detail(address,pairAddress=null,sourceHint=null){
  ...
  const holders={count:holdersCount||(sampleExact?hs.sampleCount:null),countLabel:holdersCount?String(holdersCount):(hs.sampleCount?`${hs.sampleCount}${sampleExact?"":"+"}`:null),countSource:holdersCount?"BLOCKSCOUT INDEX":(hs.sampleCount?`${hs.source} SAMPLE`:null),holderSources:bs?.holderSources||{},...hs};
  ...
  const gl=market?scorePool(market):{score:0,verdict:"UNPRICED",components:{momentum:0,liquidity:0,activity:0,flow:0,novelty:0,structure:0},model:"V3.7.3"};gl.scoreScope="MARKET_PAIR";
...
module.exports={overview,catalog,search,justBorn,detail,ohlcv,status,recentMintSignals,scorePool,CHAIN_ID,NETWORK,GLITCH,_test:{chooseSubject,normalizeDex,normalizeGT,holderStats,normalizeHolderBody,searchHolderCount,bestMarketsByToken,poolId,protocolHint}};
