...
async function fetchJson(url,timeout=9000,headers={}){
  ...
  for(let attempt=0;attempt<2;attempt++){
    ...
    try{
      const r=await fetch(url,{headers:{accept:"application/json","user-agent":"Mozilla/5.0 (compatible; DEAD-PIXELS-GLITCH-ALPHA/3.7.7; +https://www.deadpixelslabs.com/)",...headers},signal:ctrl.signal});
...
function crashGuard(p){
  const ch={m5:n(p?.priceChange?.m5),h1:n(p?.priceChange?.h1),h6:n(p?.priceChange?.h6),h24:n(p?.priceChange?.h24)};
  let cap=100,type=null,reason=null,severity=0;
  const apply=(nextCap,nextSeverity,nextType,nextReason)=>{if(nextCap<cap){cap=nextCap;type=nextType;reason=nextReason;}severity=Math.max(severity,nextSeverity);};
  // Universal downside guard. Activity/volume can NEVER override destructive price action.
  if(ch.h24<=-95)apply(10,100,"CRASH","24H PRICE COLLAPSE <= -95%");
  else if(ch.h24<=-90)apply(18,95,"CRASH","24H PRICE COLLAPSE <= -90%");
  else if(ch.h24<=-85)apply(26,90,"CRASH","24H EXTREME DRAWDOWN <= -85%");
  else if(ch.h24<=-75)apply(36,82,"DISTRESSED","24H SEVERE DRAWDOWN <= -75%");
  else if(ch.h24<=-60)apply(48,70,"DISTRESSED","24H HEAVY DRAWDOWN <= -60%");
  else if(ch.h24<=-45)apply(60,55,"WEAK","24H DRAWDOWN <= -45%");
  else if(ch.h24<=-30)apply(72,38,"WEAK","24H DRAWDOWN <= -30%");
  if(ch.h6<=-80)apply(16,96,"CRASH","6H PRICE COLLAPSE <= -80%");
  else if(ch.h6<=-65)apply(28,84,"DISTRESSED","6H SEVERE DRAWDOWN <= -65%");
  else if(ch.h6<=-50)apply(42,68,"DISTRESSED","6H HEAVY DRAWDOWN <= -50%");
  if(ch.h1<=-65)apply(22,90,"CRASH","1H PRICE COLLAPSE <= -65%");
  else if(ch.h1<=-50)apply(34,76,"DISTRESSED","1H SEVERE DRAWDOWN <= -50%");
  else if(ch.h1<=-35)apply(50,58,"WEAK","1H DRAWDOWN <= -35%");
  if(ch.m5<=-40)apply(32,74,"DISTRESSED","5M FLASH CRASH <= -40%");
  else if(ch.m5<=-25)apply(52,52,"WEAK","5M SHARP DRAWDOWN <= -25%");
  const rebound=ch.h24<=-70&&ch.h1>=15&&ch.m5>=0;
  return {active:cap<100,cap,type,reason,severity,changes:ch,rebound,reboundLabel:rebound?"REBOUND WATCH // CRASH CAP REMAINS ACTIVE":null};
}
function turnoverQuality(r){
  if(!Number.isFinite(r)||r<=0)return 0;
  if(r<.03)return 8;if(r<.10)return 20;if(r<.30)return 38;if(r<1)return 62;
  if(r<3)return 82;if(r<8)return 92;if(r<15)return 86;if(r<30)return 70;
  if(r<60)return 48;if(r<100)return 32;if(r<150)return 20;return 10;
}
function marketAnomaly(p,guard=null){
  const liq=Math.max(0,n(p?.liquidityUsd)),vol=Math.max(0,n(p?.volume?.h24));
  const ratio=liq>0?vol/liq:0,g=guard||crashGuard(p),flags=[];let a=0;
  const add=(v,msg)=>{a+=v;if(msg&&!flags.includes(msg))flags.push(msg);};
  a+=g.severity*.55;if(g.active&&g.reason)flags.push(g.reason);
  if(ratio>=150)add(35,"EXTREME 24H TURNOVER >=150X LIQUIDITY");
  else if(ratio>=100)add(30,"EXTREME 24H TURNOVER >=100X LIQUIDITY");
  else if(ratio>=50)add(22,"ABNORMAL 24H TURNOVER >=50X LIQUIDITY");
  else if(ratio>=20)add(14,"HIGH 24H TURNOVER >=20X LIQUIDITY");
  else if(ratio>=10)add(7,"ELEVATED 24H TURNOVER >=10X LIQUIDITY");
  if(liq>0&&liq<10000&&ratio>=20)add(12,"MICRO LIQUIDITY + HEAVY CHURN");
  const pc=p?.priceChange||{};
  const positivePeak=Math.max(n(pc.m5),n(pc.h1),n(pc.h6),n(pc.h24));
  if(positivePeak>=5000)add(30,"PARABOLIC PRICE MOVE >=5000%");
  else if(positivePeak>=1000)add(22,"PARABOLIC PRICE MOVE >=1000%");
  else if(positivePeak>=300)add(12,"EXTREME PRICE EXPANSION >=300%");
  const h1=p?.txns?.h1||{},b=n(h1.buys),s=n(h1.sells),tot=b+s;
  if(tot>=10&&s>=b*3)add(10,"HEAVY 1H SELL IMBALANCE");
  if(tot>=10&&b>=5&&s===0)add(10,"ONE-SIDED 1H BUY FLOW");
  const score=clamp(Math.round(a));
  return {score,level:score>=85?"EXTREME":score>=65?"HIGH":score>=40?"ELEVATED":"NORMAL",flags:flags.slice(0,8),turnoverRatio:ratio};
}
function survivalScore(p,guard=null){
  const g=guard||crashGuard(p),pc=p?.priceChange||{};
  let s=100;
  const neg=(v,w,limit)=>{const x=Math.min(limit,Math.max(0,-n(v)));s-=x*w;};
  neg(pc.m5,.75,50);neg(pc.h1,.38,100);neg(pc.h6,.16,200);neg(pc.h24,.42,100);
  // Hard survival floors align with the universal crash guard.
  if(n(pc.h24)<=-95)s=Math.min(s,2);else if(n(pc.h24)<=-90)s=Math.min(s,6);else if(n(pc.h24)<=-85)s=Math.min(s,12);else if(n(pc.h24)<=-75)s=Math.min(s,25);else if(n(pc.h24)<=-60)s=Math.min(s,42);
  if(n(pc.h6)<=-65)s=Math.min(s,20);if(n(pc.h1)<=-50)s=Math.min(s,25);if(n(pc.m5)<=-40)s=Math.min(s,30);
...
function scorePool(p){
  ...
  const pc=p.priceChange||{},m5=clamp(n(pc.m5),-60,80),h1=clamp(n(pc.h1),-120,180),h6=clamp(n(pc.h6),-250,400),h24=clamp(n(pc.h24),-400,800);
  const guard=crashGuard(p),survival=survivalScore(p,guard),anomaly=marketAnomaly(p,guard);
  let momentum=clamp(50+m5*.40+h1*.34+h6*.10+h24*.035);
  const parabolicPenalty=Math.max(0,m5-40)*.20+Math.max(0,h1-120)*.06+Math.max(0,h24-500)*.01;
  momentum=clamp(momentum-Math.min(18,parabolicPenalty));
  const depthAbs=clamp((Math.log10(liq+1)-3)*35);
  const structure=fdv>0?clamp((liq/fdv)*180):50;
  const liquidity=clamp(depthAbs*.65+structure*.35);
  const volAbs=clamp((Math.log10(vol+1)-2)*25);
  const turnoverRatio=liq>0?vol/liq:0,turnover=turnoverQuality(turnoverRatio);
  const txScore=clamp(Math.log10(tx24+1)*38);
  // Activity rewards healthy turnover, not wash-like churn. At extreme turnover the contribution drops.
  const activity=clamp(volAbs*.38+turnover*.34+txScore*.28);
  const h24buys=n(h24tx.buys),h24sells=n(h24tx.sells),h24total=h24buys+h24sells;
  const flow1=total?50+(buys-sells)/total*34:50,flow24=h24total?50+(h24buys-h24sells)/h24total*26:50;
  const flow=clamp(flow1*.65+flow24*.35);
  const age=p.ageSeconds;let maturity=50;
  if(age!=null){maturity=age<600?15:age<3600?30:age<21600?45:age<86400?60:age<604800?75:85;}
  let score=Math.round(momentum*.14+liquidity*.18+activity*.13+flow*.10+maturity*.10+structure*.10+survival*.25);
  // Universal quality gates. These are applied to EVERY token and EVERY market pair.
  if(guard.active)score=Math.min(score,guard.cap);
  if(anomaly.score>=90)score=Math.min(score,32);else if(anomaly.score>=80)score=Math.min(score,42);else if(anomaly.score>=70)score=Math.min(score,55);
  // Extreme churn on shallow liquidity is not quality alpha. This catches wash-like/bot-heavy market structure without punishing deep pools solely for high turnover.
  if(liq>0&&liq<10000&&turnoverRatio>=100)score=Math.min(score,42);
  else if(liq>0&&liq<10000&&turnoverRatio>=50)score=Math.min(score,48);
  else if(liq>0&&liq<25000&&turnoverRatio>=100)score=Math.min(score,55);
  else if(liq>0&&liq<25000&&turnoverRatio>=50)score=Math.min(score,62);
  // Low-liquidity new markets cannot rank as strong alpha merely because they pumped.
  if(liq>0&&liq<2000&&age!=null&&age<86400)score=Math.min(score,30);
  else if(liq>0&&liq<5000&&age!=null&&age<3600)score=Math.min(score,40);
  else if(liq>0&&liq<10000&&age!=null&&age<600)score=Math.min(score,46);
  // Parabolic microcaps need market depth before receiving a high opportunity score.
  if(liq<10000&&Math.max(m5,h1,h6,h24)>=300)score=Math.min(score,38);
  else if(liq<25000&&Math.max(h1,h6,h24)>=1000)score=Math.min(score,48);
  score=clamp(score);
  const marketRisk=clamp(Math.round(anomaly.score*.72+(100-survival)*.28+(liq>0&&liq<3000?15:liq>0&&liq<10000?7:0)));
  let verdict=score>=82?"ALPHA":score>=70?"HOT":score>=55?"ACTIVE":score>=40?"WATCH":"COLD";
  if(guard.type==="CRASH")verdict="CRITICAL";else if(guard.type==="DISTRESSED")verdict="DISTRESSED";else if(anomaly.score>=85)verdict="ANOMALY";
  const strengths=[],warnings=[];
  if(survival>=90)strengths.push("PRICE SURVIVAL HEALTHY");if(liquidity>=70)strengths.push("LIQUIDITY DEPTH STRONG");if(flow>=60)strengths.push("BUY FLOW POSITIVE");if(activity>=70&&anomaly.score<65)strengths.push("ACTIVITY HEALTHY");if(turnoverRatio>=.3&&turnoverRatio<15)strengths.push("TURNOVER HEALTHY");if(maturity>=75)strengths.push("MARKET MATURITY ESTABLISHED");
  if(guard.active&&guard.reason)warnings.push(guard.reason);if(guard.rebound)warnings.push(guard.reboundLabel);for(const f of anomaly.flags||[]){if(!warnings.includes(f))warnings.push(f);}if(liq>0&&liq<10000)warnings.push("LOW LIQUIDITY DEPTH");if(flow<35)warnings.push("SELL FLOW DOMINANT");
  return {score,components:{momentum:Math.round(momentum),liquidity:Math.round(liquidity),activity:Math.round(activity),flow:Math.round(flow),novelty:Math.round(maturity),structure:Math.round(structure),survival},verdict,model:"V3.7.7",guard,anomaly,marketRisk,turnoverRatio,explain:{strengths:strengths.slice(0,6),warnings:warnings.slice(0,8)}};
}
...
function qualifiedTrending(x){
  const liq=n(x.liquidityUsd),vol=n(x.volume?.h24),tx=n(x.txns?.h24?.buys)+n(x.txns?.h24?.sells),g=x.glitch||scorePool(x);
  const marketOk=liq>=5000&&vol>=2000&&tx>=12;
  // A catastrophic/distressed market belongs in Risk Radar, not the main Alpha trending board.
  return marketOk&&g.guard?.type!=="CRASH"&&g.guard?.type!=="DISTRESSED"&&n(g.anomaly?.score)<85;
}
...
function qualifiedGainer(x){const liq=n(x.liquidityUsd),vol=n(x.volume?.h24),tx=n(x.txns?.h24?.buys)+n(x.txns?.h24?.sells),h24=n(x.priceChange?.h24),g=x.glitch||scorePool(x);return h24>0&&liq>=10000&&vol>=10000&&tx>=20&&g.guard?.type!=="CRASH"&&g.guard?.type!=="DISTRESSED";}
async function overview(){
  ...
  const risk=markets.filter(x=>n(x.liquidityUsd)>=1000||n(x.volume?.h24)>=1000).sort((a,b)=>n(b.glitch?.marketRisk)-n(a.glitch?.marketRisk)||n(b.glitch?.anomaly?.score)-n(a.glitch?.anomaly?.score)).slice(0,60);
  return {chainId:CHAIN_ID,network:NETWORK,generatedAt:new Date().toISOString(),sources,count:markets.length,trending,newest:sortFeed(mergePools(...newRaw),"new").slice(0,60),gainers,volume,risk,thresholds:{trending:{minLiquidityUsd:5000,minVolume24hUsd:2000,minTx24h:12,excludes:["CRASH","DISTRESSED","EXTREME_ANOMALY"]},volume:{minLiquidityUsd:10000,minVolume24hUsd:25000,minTx24h:25},gainers:{minLiquidityUsd:10000,minVolume24hUsd:10000,minTx24h:20,requiresPositive24h:true}},rankingUnit:"MARKET_PAIR",intelligencePolicy:"PRICE_SURVIVAL_AND_LIQUIDITY_QUALITY_BEFORE_ACTIVITY"};
}
...
function riskModel(market,sec,holders,age){
  let r=0;const flags=[];const add=(v,msg)=>{r+=v;if(msg&&!flags.includes(msg))flags.push(msg);};
  ...
  const h1=market?.txns?.h1||{};if(n(h1.buys)>=5&&n(h1.sells)===0)add(12,"NO RECENT SELLS OBSERVED");if(n(h1.buys)+n(h1.sells)>=10&&n(h1.sells)>=n(h1.buys)*3)add(12,"HEAVY 1H SELL IMBALANCE");
  if(age!=null&&age<600)add(5,"EXTREMELY NEW MARKET");
  const intel=market?scorePool(market):null,guard=intel?.guard||{},anomaly=intel?.anomaly||{};
  if(guard.type==="CRASH")add(42,guard.reason||"CATASTROPHIC PRICE ACTION");else if(guard.type==="DISTRESSED")add(30,guard.reason||"SEVERE PRICE DRAWDOWN");else if(guard.type==="WEAK")add(14,guard.reason||"WEAK PRICE ACTION");
  const tr=n(intel?.turnoverRatio);if(tr>=100)add(22,"EXTREME VOLUME / LIQUIDITY TURNOVER");else if(tr>=50)add(16,"ABNORMAL VOLUME / LIQUIDITY TURNOVER");else if(tr>=20)add(9,"HIGH VOLUME / LIQUIDITY TURNOVER");
  if(n(anomaly.score)>=85)add(20,"EXTREME MARKET ANOMALY");else if(n(anomaly.score)>=65)add(12,"HIGH MARKET ANOMALY");
  const score=clamp(Math.round(r));return {score,level:score>=75?"EXTREME":score>=55?"HIGH":score>=30?"MEDIUM":"LOW",flags:flags.slice(0,10),marketRisk:intel?.marketRisk??null,anomalyScore:anomaly.score??null,note:"Risk combines contract, holder, liquidity, price-survival and anomaly signals; it is not a guarantee of safety or sellability."};
}
...
async function detail(address,pairAddress=null,sourceHint=null){
  ...
  const gl=market?scorePool(market):{score:0,verdict:"UNPRICED",components:{momentum:0,liquidity:0,activity:0,flow:0,novelty:0,structure:0,survival:0},model:"V3.7.7"};gl.scoreScope="MARKET_PAIR";
...
module.exports={overview,catalog,search,justBorn,detail,ohlcv,status,recentMintSignals,scorePool,CHAIN_ID,NETWORK,GLITCH,_test:{chooseSubject,normalizeDex,normalizeGT,holderStats,normalizeHolderBody,searchHolderCount,bestMarketsByToken,poolId,protocolHint,dexPaidIntel,getBlockscoutDetail,crashGuard,marketAnomaly,survivalScore,turnoverQuality,qualifiedTrending,qualifiedVolume,qualifiedGainer,riskModel}};
